...
Windowsのデフォルトのイベントログ設定を改善することを強く推奨し、最も正確な情報を提供できるよう最善を尽くします。
...
`Security`、`System`、`Application`などのクラッシックイベントログの最大ファイルサイズを増やすのは簡単ですが、残念ながら、他のイベントログの最大ファイルサイズを変更するには、管理用テンプレートをインストールするか、レジストリを直接変更する必要があります。
...
このログを確認すると、Windowsのコード整合性チェックでブロックされたドライバーのロードイベントを検出することができ、ロードに失敗した悪意のあるドライバーを示すことがあります。
